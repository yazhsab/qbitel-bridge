...
class IntegratedProtocolDiscoveryOrchestrator:
    ...
    async def discover_protocol(
        self, 
        request: Union[BaseDiscoveryRequest, IntegratedDiscoveryRequest]
        ...
        use_enhanced = (
            isinstance(request, IntegratedDiscoveryRequest) and
            self._components_initialized and
            (
                request.generate_api_specification or
                request.generate_sdks or
                request.enable_semantic_analysis
            )
        )
    ...
    async def _generate_sdks(
        ...
        for language in request.target_languages:
            try:
                package_name = (
                    request.package_name or 
                    f"{api_specification.title.lower().replace(' ', '-')}-{language.value}-sdk"
                )
