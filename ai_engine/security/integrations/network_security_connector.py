...
class NetworkSecurityConnector(BaseIntegrationConnector, ABC):
    ...
    def __init__(self, config):
        ...
        self.logger = get_security_logger("cronos.security.integrations.network_security")
...
class FirewallConnector(NetworkSecurityConnector):
    ...
    async def send_security_event(self, security_event: SecurityEvent) -> IntegrationResult:
        ...
        try:
            ...
            if security_event.source_ip and security_event.threat_level.value in ['critical', 'high']:
                    ...
                    if response.status == 200:
                        ...
                        if root.get('status') == 'success':
                            ...
                            if commit_result:
                                ...
                                if duration_minutes:
                                    asyncio.create_task(
                                        self._schedule_unblock(ip_address, duration_minutes * 60)
                                    )
    ...
    async def create_firewall_rule(self, rule_config: Dict[str, Any]) -> IntegrationResult:
        ...
        rule_name = rule_config.get('name', f"CRONOS_RULE_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}")
...
class IDSConnector(NetworkSecurityConnector):
    ...
    async def create_firewall_rule(self, rule_config: Dict[str, Any]) -> IntegrationResult:
        ...
        rule_name = rule_config.get('name', f"CRONOS_IDS_RULE_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}")
