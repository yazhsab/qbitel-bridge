...
class SlackConnector(CommunicationConnector):
    ...
    def _create_response_execution_blocks(self, automated_response: AutomatedResponse) -> List[Dict]:
        ...
        status_emoji = "✅" if automated_response.status.value == "completed" else "⏳" if automated_response.status.value == "in_progress" else "❌"
        ...
        for action in automated_response.actions[:5]:  # Show first 5 actions
            status_icon = "✅" if action.status.value == "completed" else "⏳" if action.status.value == "in_progress" else "❌"
    ...
    def _get_channel_for_severity(self, severity: str) -> str:
        ...
        return channel_mapping.get(
            severity,
            channel_mapping.get('default', self.default_channel)
        )
...
class EmailConnector(CommunicationConnector):
    ...
    def _create_response_execution_email(self, automated_response: AutomatedResponse) -> str:
        ...
        status_color = "#28a745" if automated_response.status.value == "completed" else "#ffc107" if automated_response.status.value == "in_progress" else "#dc3545"
        ...
        if automated_response.actions:
            ...
            for action in automated_response.actions:
                status_icon = "✅" if action.status.value == "completed" else "⏳" if action.status.value == "in_progress" else "❌"
                actions_list.append(f"<li>{status_icon} <strong>{action.action_type.value}</strong> on {action.target_system} - {action.status.value}</li>")
    ...
    def _get_recipients_for_severity(self, severity: str) -> List[str]:
        ...
        return recipient_mapping.get(
            severity,
            recipient_mapping.get('default', self.default_recipients)
        )
