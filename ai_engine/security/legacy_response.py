...
from .models import (
    SecurityEvent, LegacySystem, ResponseAction, QuarantineResult, 
    AutomatedResponse, SystemCriticality, ProtocolType, ResponseType,
    ThreatLevel, SecurityException, QuarantineException, ResponseExecutionException,
...
# Prometheus metrics
QUARANTINE_COUNTER = Counter('cronos_security_quarantine_total', 'Quarantine operations', ['system_type', 'status'])
QUARANTINE_DURATION = Histogram('cronos_security_quarantine_duration_seconds', 'Quarantine operation duration')
LEGACY_RESPONSE_COUNTER = Counter('cronos_security_legacy_responses_total', 'Legacy responses executed', ['protocol', 'action'])
SAFETY_VIOLATIONS = Counter('cronos_security_safety_violations_total', 'Safety constraint violations', ['constraint_type'])
DEPENDENCY_CHECKS = Summary('cronos_security_dependency_check_duration_seconds', 'Dependency check duration')
...
class LegacySystemCapability:
    ...
    def _add_hl7_capabilities(self):
        ...
        self.safety_requirements.extend([
            ...
            "Preserve clinical workflow continuity"
        ])
    ...
    def _add_tn3270e_capabilities(self):
        ...
        self.safety_requirements.extend([
            ...
            "Maintain mainframe connectivity"
        ])
    ...
    def _load_safety_rules(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'protocol_constraints': {
                ProtocolType.MODBUS: {
                    'prohibited_actions': [ResponseType.NETWORK_SEGMENTATION, ResponseType.ISOLATE_SYSTEM],
    ...
    async def _execute_actions_safely(
        ...
        for action in sorted_actions:
            try:
                ...
                # Update metrics
                for system_id in action.target_systems:
                    system = next((s for cap in capabilities.values() for s in [cap.system] 
                                 if s.system_id == system_id), None)
                    if system:
        ...
        results['overall_success'] = (
            results['successful_actions'] > 0 and 
            results['failed_actions'] == 0
        )
...
Provide the plan in JSON format with isolation_method, duration, safety_checks, and rollback_plan."""
    ...
    async def _validate_quarantine_safety(
        ...
        if system.criticality == SystemCriticality.MISSION_CRITICAL:
            if plan.get('isolation_method') in ['complete_isolation', 'system_shutdown']:
