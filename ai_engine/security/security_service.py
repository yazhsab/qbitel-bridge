...
from .models import (
    ...
    LegacySystem, QuarantineResult, SecurityMetrics, ThreatIntelligence,
...
# Prometheus metrics
SECURITY_EVENTS_COUNTER = Counter('cronos_security_events_total', 'Total security events processed', ['event_type', 'severity'])
RESPONSE_TIME_HISTOGRAM = Histogram('cronos_security_response_time_seconds', 'Security response time', ['response_type'])
ACTIVE_INCIDENTS_GAUGE = Gauge('cronos_security_active_incidents', 'Number of active security incidents')
AUTONOMOUS_DECISIONS_COUNTER = Counter('cronos_security_autonomous_decisions_total', 'Autonomous decisions made', ['decision_outcome'])
HUMAN_ESCALATIONS_COUNTER = Counter('cronos_security_human_escalations_total', 'Human escalations', ['escalation_reason'])
SYSTEM_QUARANTINES_GAUGE = Gauge('cronos_security_active_quarantines', 'Number of systems currently quarantined')
MTTR_SUMMARY = Summary('cronos_security_mean_time_to_response_seconds', 'Mean time to response for security incidents')
...
class SecurityOrchestratorService:
    ...
    async def get_incident_status(self, incident_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'event_id': incident_data.get('event', {}).event_id if hasattr(incident_data.get('event', {}), 'event_id') else None,
        ...
        for incident_id, incident_data in self.active_incidents.items():
            if incident_data.get('status') in ['processing', 'pending_approval']:
                active_list.append({
                    'incident_id': incident_id,
                    'status': incident_data.get('status'),
                    'event_type': getattr(incident_data.get('event'), 'event_type', {}).value if hasattr(incident_data.get('event', {}), 'event_type') else 'unknown',
                    'threat_level': getattr(incident_data.get('event'), 'threat_level', {}).value if hasattr(incident_data.get('event', {}), 'threat_level') else 'unknown',
                    'start_time': incident_data.get('start_time'),
                    'duration': time.time() - incident_data.get('start_time', time.time())
                })
    ...
    async def _cleanup_old_incidents(self) -> None:
        ...
        for incident_id, incident_data in self.active_incidents.items():
            if (incident_data.get('status') in ['completed', 'failed'] and 
                incident_data.get('start_time', 0) < cutoff_time):
    ...
    async def _update_security_context_automatically(self) -> None:
        ...
        active_incident_ids = [
            iid for iid, data in self.active_incidents.items()
    ...
    async def _update_security_context(self, security_event: SecurityEvent) -> None:
        ...
        self.security_context.recent_attacks = [
            attack for attack in self.security_context.recent_attacks
...
async def initialize_security_service(config: Optional[Config] = None) -> SecurityOrchestratorService:
