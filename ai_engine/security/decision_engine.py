...
from .models import (
    ...
    validate_confidence_score, calculate_risk_score
)
...
# Prometheus metrics
DECISION_COUNTER = Counter('cronos_security_decisions_total', 'Security decisions made', ['decision_type', 'confidence_level'])
DECISION_DURATION = Histogram('cronos_security_decision_duration_seconds', 'Decision making duration')
RESPONSE_EXECUTION_COUNTER = Counter('cronos_security_responses_total', 'Security responses executed', ['response_type', 'status'])
AUTO_EXECUTE_GAUGE = Gauge('cronos_security_auto_executions', 'Autonomous executions per hour')
HUMAN_ESCALATIONS_GAUGE = Gauge('cronos_security_human_escalations', 'Human escalations per hour')
    ...
    async def analyze_and_respond(
        ...
        try:
            ...
            decision = await self._make_llm_decision(
                security_event, threat_analysis, business_impact, response_options, security_context
            )
            ...
            for system in legacy_systems:
                if system.system_id in security_event.affected_systems:
                    ...
                    if system.criticality.value == 'mission_critical':
                        ...
                        if business_impact['operational_impact'] not in ['critical', 'high']:
    ...
    async def _create_response_plan(
        ...
        if not any(action.action_type == ResponseType.ALERT_SECURITY_TEAM for action in response_actions):
            alert_action = await self._create_response_action(
                ResponseType.ALERT_SECURITY_TEAM, security_event, threat_analysis, legacy_systems
            )
    ...
    async def _apply_safety_constraints(
        ...
        if not is_business_hours:
            for action in validated_actions:
                if action.action_type in {ResponseType.SHUTDOWN_SERVICE, ResponseType.PATCH_VULNERABILITY}:
    ...
    def _apply_legacy_constraints(self, action: ResponseAction, legacy_systems: List[LegacySystem]):
        ...
        for system in legacy_systems:
            if system.system_id in action.target_systems:
                ...
                if system.criticality in {SystemCriticality.MISSION_CRITICAL, SystemCriticality.BUSINESS_CRITICAL}:
                    action.requires_approval = True
...
Focus on actionable intelligence and context."""
        ...
        try:
            ...
            return {'analysis': f'LLM analysis failed: {e}', 'contextual_factors': [], 'similar_incidents': []}
...
Provide realistic estimates and context."""
    ...
    def _parse_business_impact_response(self, response_text: str, current_impact: Dict[str, Any]) -> Dict[str, Any]:
        ...
        import re
        financial_patterns = [r'\$([0-9,]+)', r'([0-9,]+)\s*dollars?', r'cost.*?([0-9,]+)']
    ...
    def _get_base_responses(self, event_type: SecurityEventType, threat_level: ThreatLevel) -> List[ResponseType]:
        ...
        base_responses = [ResponseType.ALERT_SECURITY_TEAM, ResponseType.ENABLE_MONITORING]
        
        # Add responses based on event type
        event_responses = {
            SecurityEventType.MALWARE_DETECTION: [ResponseType.QUARANTINE, ResponseType.BACKUP_SYSTEM],
            SecurityEventType.INTRUSION_ATTEMPT: [ResponseType.BLOCK_IP, ResponseType.NETWORK_SEGMENTATION],
            SecurityEventType.DATA_EXFILTRATION: [ResponseType.ISOLATE_SYSTEM, ResponseType.LOG_RETENTION_INCREASE],
            SecurityEventType.VULNERABILITY_EXPLOIT: [ResponseType.VIRTUAL_PATCH, ResponseType.PATCH_VULNERABILITY],
            SecurityEventType.RANSOMWARE_ACTIVITY: [ResponseType.ISOLATE_SYSTEM, ResponseType.BACKUP_SYSTEM],
            SecurityEventType.INSIDER_THREAT: [ResponseType.RESET_CREDENTIALS, ResponseType.LOG_RETENTION_INCREASE]
        }
    ...
    def _assess_legacy_considerations(self, response_type: ResponseType, legacy_systems: Optional[List[LegacySystem]]) -> Dict[str, Any]:
    ...
    async def get_decision_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_confidence': sum(d.get('confidence', 0) for d in self.decision_history) / max(len(self.decision_history), 1),
