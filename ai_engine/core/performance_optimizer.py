...
class BatchProcessor:
    ...
    async def add_item(self, processor_id: str, item: Any, process_func: Callable):
        ...
        async with self._lock:
            ...
            current_time = time.time()
            should_flush = (
                len(processor['batch']) >= self.batch_size or
                current_time - processor['last_flush'] >= self.flush_interval
            )
    ...
    async def performance_context(self):
        ...
        try:
            ...
            self._metrics.throughput_per_sec = (
                self._metrics.total_requests / (time.time() - self._start_time)
            )
        ...
        health_status = {
            ...
            'caches': {
                name: {
                    ...
                    'size': cache.get('pool_size', cache.get('hits', 0)) if isinstance(cache, dict) else getattr(cache, 'hits', 0)
                }
