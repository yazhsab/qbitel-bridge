...
class RetryConfig:
    ...
    retryable_exceptions: Set[Type[Exception]] = field(
        default_factory=lambda: {ConnectionError, TimeoutError, InferenceException}
    )
...
class ErrorHandler:
    ...
    def _initialize_classification_rules(
        self,
    ) -> Dict[Type[Exception], Tuple[ErrorSeverity, ErrorCategory, RecoveryStrategy]]:
    ...
    def classify_error(
        ...
        for base_type, (
            ...
            category,
            strategy,
        ) in self.classification_rules.items():
...
@asynccontextmanager
async def error_context(component: str, operation: str, **context_data):
    """
