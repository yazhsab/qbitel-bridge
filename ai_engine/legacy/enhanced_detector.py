...
class LegacySystemLLMAnalyzer:
    ...
    async def predict_failure_with_context(
        ...
        try:
            ...
            return {
                "failure_prediction": {"error": str(e)},
                "confidence": 0.0
            }
...
""")
    ...
    def _extract_failure_type(self, content: str) -> str:
        ...
        content_lower = content.lower()
        failure_types = [
            "hardware_failure", "software_corruption", "performance_degradation",
            "memory_leak", "disk_failure", "network_timeout", "configuration_drift",
            "security_breach", "data_corruption", "capacity_overrun"
        ]
...
class EnhancedLegacySystemDetector(VAEAnomalyDetector):
        ...
        try:
            ...
            if self.llm_analyzer:
                failure_prediction = await self.llm_analyzer.predict_failure_with_context(
                    ...
                    system_context=system_context or LegacySystemContext(
                        system_id="unknown",
    ...
    def get_system_health_summary(self, system_id: str) -> Dict[str, Any]:
        ...
        recent_predictions = [
            ...
            if pred.system_id == system_id and 
            (datetime.now() - pred.prediction_timestamp).total_seconds() < 3600 * 24  # Last 24 hours
        ]
