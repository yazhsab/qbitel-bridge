...
from ..models import (
    ...
    SystemMetrics, TimeSeriesData, PredictionHorizon
)
...
class TestLegacySystemWhispererService:
    ...
    async def test_schedule_maintenance_success(self, initialized_service):
        ...
        initialized_service.maintenance_scheduler.optimize_maintenance_schedule = AsyncMock(
    ...
    async def test_error_handling_initialization_failure(self, service, mock_llm_service):
        ...
        with patch.object(service, '_initialize_components', side_effect=Exception("Init failed")):
            with pytest.raises(CronosAIException, match="Service initialization failed"):
