...
from ..config import (
    # Enums
    LegacySystemType, PredictionHorizonConfig, KnowledgeSourceType,
    
    # Config Classes
    AnomalyDetectionConfig, PredictiveAnalyticsConfig, KnowledgeCaptureConfig,
    DecisionSupportConfig, LLMIntegrationConfig, MonitoringObservabilityConfig,
    SecurityConfig, LegacySystemWhispererConfig,
    
    # Factory functions
    extend_base_config_with_legacy, load_legacy_config_from_file,
    load_legacy_config_from_env, get_default_legacy_config,
    validate_legacy_config, create_production_legacy_config,
...
class TestAnomalyDetectionConfig:
    ...
    def test_anomaly_config_validation_invalid_threshold(self):
        ...
        with pytest.raises(ConfigurationException, match="Anomaly threshold must be between 0.5 and 1.0"):
            config.validate()
...
class TestPredictiveAnalyticsConfig:
    ...
    def test_predictive_config_validation_insufficient_historical_data(self):
        ...
        with pytest.raises(ConfigurationException, match="Minimum historical data must be at least 7 days"):
            config.validate()
    ...
    def test_predictive_config_validation_invalid_confidence_threshold(self):
        ...
        with pytest.raises(ConfigurationException, match="Prediction confidence threshold must be between 0.5 and 1.0"):
            config.validate()
    ...
    def test_predictive_config_validation_invalid_resource_utilization(self):
        ...
        with pytest.raises(ConfigurationException, match="Resource utilization target must be between 0.0 and 1.0"):
            config.validate()
...
class TestKnowledgeCaptureConfig:
    ...
    def test_knowledge_config_validation_invalid_validation_threshold(self):
        ...
        with pytest.raises(ConfigurationException, match="Knowledge validation threshold must be between 0.0 and 1.0"):
            config.validate()
...
class TestDecisionSupportConfig:
    ...
    def test_decision_config_validation_invalid_diversity(self):
        ...
        with pytest.raises(ConfigurationException, match="Recommendation diversity must be between 0.0 and 1.0"):
            config.validate()
...
class TestConfigPerformance:
    ...
    def test_config_creation_performance(self):
        ...
        for i in range(100):
            config = LegacySystemWhispererConfig(
                service_name=f"perf_test_{i}",
                max_registered_systems=100 + i
            )
