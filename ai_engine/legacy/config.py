...
def load_legacy_config_from_file(
...
def load_legacy_config_from_env(
    ...
    if os.getenv(f"{prefix}PREDICTION_ENABLED"):
        config.predictive_analytics.failure_prediction_enabled = (
            os.getenv(
                f"{prefix}PREDICTION_ENABLED",
                str(config.predictive_analytics.failure_prediction_enabled),
            ).lower()
            == "true"
        )
