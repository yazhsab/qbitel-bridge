...
class ProtocolDiscoveryBenchmark:
    ...
    def __init__(
        self,
        discovery_system: ProductionProtocolDiscovery,
        config: Config
    ):
    ...
    async def run_latency_benchmark(
        ...
        for i in range(config.num_requests):
            ...
            try:
                await self.discovery_system.discover_with_sla(
                    request,
                    sla_ms=config.sla_threshold_ms
                )
        ...
        self.logger.info(f"Latency benchmark completed: p50={latency_metrics.p50_ms:.2f}ms, "
    ...
    async def run_accuracy_benchmark(
        ...
        confusion_matrix = {
            protocol: {p: 0 for p in unique_protocols}
    ...
    async def run_resource_usage_benchmark(
        ...
        self.results.append(result)
        self.logger.info(f"Resource usage benchmark completed: "
    ...
    async def run_stress_test(
        ...
        for time_point, concurrent_load in load_schedule:
            ...
            for i in range(concurrent_load):
                ...
                tasks.append(
                    self.discovery_system.discover_with_sla(request, config.sla_threshold_ms)
                )
    ...
    def generate_report(self) -> str:
        ...
        report_lines = [
            "=" * 80,
            "PROTOCOL DISCOVERY BENCHMARK REPORT",
            "=" * 80,
            ""
        ]
...
(self.metrics["cpu_percent"]) if self.metrics["cpu_percent"] else 0.0,
            ...
            gpu_utilization_mean=statistics.mean(self.metrics["gpu_utilization"]) if self.metrics["gpu_utilization"] else None
        )