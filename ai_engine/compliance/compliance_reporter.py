    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            "report_generation": {
                ...
                "target_met": sum(1 for t in self.report_generation_times if t < 600) / len(self.report_generation_times) * 100 if self.report_generation_times else 0
            },
            "compliance_accuracy": {
                ...
                "target_met": sum(1 for s in self.compliance_accuracy_scores if s >= 95.0) / len(self.compliance_accuracy_scores) * 100 if self.compliance_accuracy_scores else 0
            },
            "audit_pass_rate": {
                "average_rate": np.mean(self.audit_pass_rates) if self.audit_pass_rates else 0,
                "total_audits": len(self.audit_pass_rates),
                "target_met": sum(1 for r in self.audit_pass_rates if r >= 98.0) / len(self.audit_pass_rates) * 100 if self.audit_pass_rates else 0
            },
            "monitoring": {
...
async def get_compliance_reporter(config: Optional[Config] = None) -> AutonomousComplianceReporter:
    """
