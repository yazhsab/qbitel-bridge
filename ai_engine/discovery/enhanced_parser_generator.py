...
from .parser_generator import (
    ParseResult, ParserNode, GeneratedParser, ParserTemplate, ParserGenerator
)
    ...
    async def _eliminate_dead_code(
        ...
        for method_name in method_names:
            if method_name.startswith('parse_'):
                ...
                is_reachable = any(
                    symbol_name in s or s in symbol_name
                    for s in reachable_symbols
                )
                if not is_reachable:
        ...
        try:
            if format == 'json':
                ...
                with open(filepath, 'w', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow(['Parser ID', 'Throughput', 'Success Rate', 'Memory (MB)', 'Avg Time (ms)'])
