...
from .protocol_classifier import ProtocolClassifier, ClassificationResult, ProtocolSample
...
# Prometheus metrics
DISCOVERY_COUNTER = PrometheusCounter('cronos_protocol_discovery_total', 'Total protocol discoveries', ['protocol', 'status'])
DISCOVERY_DURATION = Histogram('cronos_protocol_discovery_duration_seconds', 'Discovery duration', ['phase'])
...
class ProtocolDiscoveryOrchestrator:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            initialization_tasks = [
                self.statistical_analyzer.initialize() if hasattr(self.statistical_analyzer, 'initialize') else self._dummy_init(),
                ...
                self.protocol_classifier.initialize() if hasattr(self.protocol_classifier, 'initialize') else self._dummy_init(),
    ...
    async def _execute_discovery(self, request: DiscoveryRequest) -> DiscoveryResult:
        ...
        try:
            ...
            statistical_result = await self.statistical_analyzer.analyze_messages(request.messages)
            result.statistical_analysis = statistical_result
            
            phase_time = time.time() - phase_start
            self.discovery_stats['phase_timings']['statistical_analysis'].append(phase_time)
            ...
            if not request.known_protocol:
                ...
                if request.messages and self.protocol_classifier.is_trained:
                    classification_result = await self.protocol_classifier.classify(request.messages[0])
            ...
            if request.generate_parser and result.grammar:
                ...
                phase_time = time.time() - phase_start
                self.discovery_stats['phase_timings']['parser_generation'].append(phase_time)
            ...
            self.discovery_stats['average_discovery_time'] = (
                (avg_time * (total - 1) + result.processing_time) / total
            )
    ...
    async def get_protocol_profiles(self) -> Dict[str, Dict[str, Any]]:
        ...
        for name, profile in self.protocol_profiles.items():
            avg_confidence = (
                ...
                if profile.confidence_scores else 0.0
            )
