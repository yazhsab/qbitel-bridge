...
class StatisticalAnalyzer:
    ...
    async def _compute_byte_statistics(self, data: bytes) -> ByteStatistics:
        ...
        if not data:
            return ByteStatistics(
                frequency={}, total_count=0, entropy=0.0, mean=0.0,
                std_dev=0.0, skewness=0.0, kurtosis=0.0, chi_square_stat=0.0,
                chi_square_pvalue=1.0, is_random=False, is_text=False, is_binary=True
            )
    ...
    def _is_likely_text(self, data: bytes) -> bool:
        ...
        try:
            ...
            text_bytes = set(range(32, 127)) | {9, 10, 13}  # Printable ASCII + tab, LF, CR
    ...
    async def _find_common_suffixes(self, messages: List[bytes]) -> List[Tuple[bytes, int]]:
        ...
        significant_suffixes = [
            (suffix, count) for suffix, count in suffix_counts.items()
            if count >= threshold
        ]
    ...
    def _calculate_pattern_significance(self, pattern: bytes, occurrences: List[Tuple[int, int]], messages: List[bytes]) -> float:
