...
class ProtocolDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            "mask": torch.tensor([1] * len(message) + [0] * (self.max_length - len(message)), dtype=torch.bool)
        }
...
class EnhancedGrammarLearner:
    ...
    async def learn_hierarchical_grammar(
        self,
        messages: List[bytes],
        layer_hints: Optional[Dict[str, List[int]]] = None
    ) -> HierarchicalGrammar:
        """
    ...
    async def learn_with_transfer_learning(
        ...
        try:
            ...
            adapted_grammar = await self._adapt_grammar(
                transfer_model.source_grammar,
                messages[:adaptation_samples]
            )
            
            # Step 4: Fine-tune with target samples
            final_grammar = await self._fine_tune_grammar(
                adapted_grammar,
                messages
            )
    ...
    async def evaluate_grammar(
        ...
        try:
            ...
            self.logger.info(f"Evaluation complete: Accuracy={metrics['accuracy']:.3f}, "
    ...
    async def _detect_protocol_layers(
        self,
        messages: List[bytes],
        layer_hints: Optional[Dict[str, List[int]]]
    ) -> Dict[str, List[bytes]]:
    ...
    async def _learn_inter_layer_rules(
        self,
        layers: Dict[str, List[bytes]],
        layer_grammars: Dict[str, Grammar]
    ) -> List[ProductionRule]:
    ...
    async def _check_learning_confidence(
        ...
        parsed_count = sum(
            1 for msg in sample_messages
            if await self._can_parse_message(grammar, msg)
        )
            ...
            for layer_name, layer_grammar in grammar.layers.items():
                ...
                for rule in layer_grammar.rules[:20]:  # Limit for readability
                    ...
                    for rhs_symbol in rule.right_hand_side:
                        G.add_edge(lhs, rhs_symbol.name, 
                                 probability=rule.probability,
                                 layer=layer_name)
...
"""
        ...
        for rule in grammar.rules[:100]:  # Limit for performance
            rhs_str = " ".join([f'<span class="symbol">{s.name}</span>' 
                               for s in rule.right_hand_side])
            html += f"""
    <div class="rule">
...
"""
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "average_sample_efficiency": np.mean(self.metrics["sample_efficiency"]) if self.metrics["sample_efficiency"] else 0.0,
...
def merge_grammars(grammars: List[Grammar], weights: Optional[List[float]] = None) -> Grammar:
    ...
    for grammar, weight in zip(grammars, weights):
        for rule in grammar.rules:
            rule_key = (rule.left_hand_side.name, 
    ...
    for rule_key, rule_weight_pairs in rule_map.items():
        ...
        avg_prob = sum(rule.probability * weight 
                      for rule, weight in rule_weight_pairs) / len(rule_weight_pairs)
...
def simplify_grammar(grammar: Grammar, min_probability: float = 0.01) -> Grammar:
    ...
    simplified_symbols = {
        name: symbol for name, symbol in grammar.symbols.items()
