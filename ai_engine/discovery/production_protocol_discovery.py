...
class ExplainableDiscovery:
    ...
    def _generate_confidence_breakdown(
        self, prediction: ModelOutput, feature_importances: List[FeatureImportance]
    ) -> Dict[str, float]:
...
class ModelVersionManager:
    ...
    def promote_canary(self, version_id: str) -> bool:
        ...
        if not self._validate_canary_performance(version_id):
            self.logger.warning(f"Canary {version_id} performance validation failed")
            return False
    ...
    def select_version_for_request(self, request: DiscoveryRequest) -> str:
...
class ProductionProtocolDiscovery:
    ...
    async def discover_with_versioning(
        ...
        result = await self.discover_with_sla(request, sla_ms=request.sla_ms or 200)
    ...
    async def _perform_discovery(
        ...
        # Prepare input
        model_input = ModelInput(data=request.packet_data, metadata=request.metadata)
    ...
    async def _handle_sla_violation(
        ...
        if fallback_strategy == FallbackStrategy.CACHED_RESULT:
            ...
            raise SLAViolationException(f"SLA violation: exceeded {sla_ms}ms threshold")
    ...
    def _extract_structure(self, prediction: ModelOutput) -> Dict[str, Any]:
        ...
        return {"fields": [], "format": "binary", "version": "unknown"}
