...
class BayesianOptimizer:
    ...
    async def optimize_hyperparameters(
        ...
        for i in range(n_random_starts):
            ...
            self.logger.debug(f"Random start {i+1}/{n_random_starts}",
                            score=score)
        ...
        for i in range(n_iterations - n_random_starts):
            ...
            self.logger.debug(f"Bayesian iteration {i+1}/{n_iterations-n_random_starts}",
        ...
        for param, (low, high) in param_space.items():
            ...
            if param in ['min_pattern_frequency', 'max_rule_length', 'max_iterations']:
                new_value = int(np.clip(
                    current_value + np.random.normal(0, noise_scale),
                    low, high
                ))
            else:
                new_value = np.clip(
...
class EnhancedPCFGInference:
    ...
    def __init__(
        ...
        self.logger.info("Enhanced PCFG Inference initialized",
    ...
    async def parallel_grammar_inference(
        ...
        duration = time.time() - start_time
        self.logger.info(f"Parallel inference completed",
                        duration=duration,
    ...
    def _has_converged_advanced(self, convergence_scores: List[float]) -> bool:
        ...
        if len(convergence_scores) >= 3:
            if all(convergence_scores[i] <= convergence_scores[i-1] * 1.01
                   for i in range(-2, 0)):
                return True
