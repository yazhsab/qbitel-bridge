...
class LSTMProtocolClassifier(nn.Module):
    ...
    def __init__(self, input_size: int, num_classes: int, embedding_dim: int = 64, hidden_dim: int = 128):
...
class ProtocolClassifier:
    ...
    async def train(
        ...
        try:
            ...
            X_train, X_val, y_train, y_val = train_test_split(
                features, labels_encoded, test_size=validation_split, 
            ...
            training_tasks.append(
                self._train_cnn_model(training_samples, X_train, y_train, X_val, y_val, epochs)
            )
            
            # Train LSTM model
            training_tasks.append(
                self._train_lstm_model(training_samples, X_train, y_train, X_val, y_val, epochs)
            )
    ...
    async def classify(self, data: bytes) -> ClassificationResult:
        ...
        try:
            ...
            # Ensemble prediction
            final_prediction, final_confidence, final_probabilities = await self._ensemble_predict(
    ...
    async def _train_cnn_model(
        ...
        try:
            ...
            train_sequences = await self._prepare_sequences([s.data for s in samples if s.label in self.label_encoder.transform([s.label])])
