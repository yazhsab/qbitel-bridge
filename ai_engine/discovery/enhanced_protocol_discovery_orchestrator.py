...
from ..copilot.protocol_copilot import ProtocolIntelligenceCopilot, CopilotQuery, CopilotResponse
...
class EnhancedProtocolDiscoveryOrchestrator(ProtocolDiscoveryOrchestrator):
    ...
    def __init__(self, config: Config, protocol_copilot: Optional[ProtocolIntelligenceCopilot] = None):
    ...
    async def _generate_natural_language_summary(self, result: EnhancedDiscoveryResult) -> None:
        ...
        try:
            ...
            if any(analysis_type in [LLMAnalysisType.SECURITY_ASSESSMENT, LLMAnalysisType.VULNERABILITY_ANALYSIS] 
    ...
    def _extract_security_summary(self, analyses: Dict[LLMAnalysisType, LLMAnalysisResult]) -> str:
        ...
        security_analyses = [
            analysis for analysis_type, analysis in analyses.items()
            if analysis_type in [LLMAnalysisType.SECURITY_ASSESSMENT, LLMAnalysisType.VULNERABILITY_ANALYSIS]
        ]
        ...
        high_confidence_findings = [
            analysis.response for analysis in security_analyses 
    ...
    async def get_enhanced_protocol_profiles(self) -> Dict[str, Dict[str, Any]]:
        ...
        for protocol_name, profile in base_profiles.items():
            if protocol_name in self.protocol_profiles:
                ...
                recent_analyses = sum(
                    1 for cache_analyses in self.llm_analysis_cache.values()
