...
# Metrics
KB_DOCUMENT_COUNTER = Counter('cronos_kb_documents_total', 'Total knowledge base documents', ['collection'])
KB_QUERY_COUNTER = Counter('cronos_kb_queries_total', 'Total knowledge base queries', ['type'])
KB_UPDATE_DURATION = Histogram('cronos_kb_update_duration_seconds', 'Knowledge base update duration')
...
class ProtocolKnowledgeBase:
        ...
        try:
            ...
            export_data = {
                ...
                "interaction_stats": {
                    ...
                    "avg_confidence": sum(i.confidence for i in self.interaction_history) / len(self.interaction_history) if self.interaction_history else 0.0,
            ...
            base_protocols = [
                ProtocolKnowledge(
                    ...
                    field_definitions=[
                        ...
                        {"name": "uri", "type": "string", "description": "Resource identifier"},
                        {"name": "version", "type": "string", "description": "HTTP version"},
                        {"name": "headers", "type": "key-value", "description": "Request/response headers"},
                        {"name": "body", "type": "bytes", "description": "Message body"}
                    ],
                    common_patterns=[
                ...
                ProtocolKnowledge(
                    ...
                    technical_details={
                        ...
                        "security_extensions": ["DNSSEC", "DNS over HTTPS (DoH)", "DNS over TLS (DoT)"]
                    },
                    security_implications=[
                    ...
                    field_definitions=[
                        {"name": "header", "type": "struct", "description": "DNS message header"},
                        {"name": "question", "type": "struct", "description": "Query information"},
                        {"name": "answer", "type": "resource_record", "description": "Response records"},
                        {"name": "authority", "type": "resource_record", "description": "Authority records"},
                        {"name": "additional", "type": "resource_record", "description": "Additional records"}
                    ],
                    common_patterns=[
    ...
    async def _extract_knowledge_from_interaction(self, interaction: InteractionRecord) -> None:
        ...
        try:
            ...
            protocols = ['http', 'https', 'tcp', 'udp', 'dns', 'ftp', 'ssh', 'tls', 'ssl']
        ...
        try:
            ...
            for interaction in recent_interactions:
                ...
                if interaction.protocol_context:
                    protocol_mentions[interaction.protocol_context] = protocol_mentions.get(interaction.protocol_context, 0) + 1
