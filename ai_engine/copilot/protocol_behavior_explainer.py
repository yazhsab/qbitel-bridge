...
class ProtocolBehaviorExplainer:
    ...
    def _analyze_message_characteristics(self, messages: List[bytes]) -> Dict[str, Any]:
    ...
    async def _get_protocol_knowledge(
        ...
        try:
            ...
            return [
                {"content": doc.content, "metadata": doc.metadata, "similarity": score}
    ...
    async def _identify_security_implications(
        self, protocol_type: str, patterns: List[MessagePattern], messages: List[bytes]
    ) -> List[str]:
        """Identify security implications."""
        implications = []
        ...
        if text_messages > len(messages) * 0.5:
            implications.append("Messages appear to contain unencrypted text data")
