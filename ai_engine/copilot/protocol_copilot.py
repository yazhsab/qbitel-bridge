...
# Metrics
COPILOT_QUERY_COUNTER = Counter('cronos_copilot_queries_total', 'Total copilot queries', ['query_type', 'status'])
COPILOT_QUERY_DURATION = Histogram('cronos_copilot_query_duration_seconds', 'Copilot query duration', ['query_type'])
COPILOT_CONFIDENCE_SCORE = Histogram('cronos_copilot_confidence_score', 'Copilot confidence scores')
...
{immediate_actions}
    ...
    async def process_query(self, query: CopilotQuery) -> CopilotResponse:
        ...
        try:
            ...
            enhanced_context = await self.rag_engine.enhance_query_context(
                query.query,
                {**conversation_context, **(query.context or {})}
            )
    ...
    def _format_context_for_llm(self, context: Dict[str, Any]) -> str:
        ...
        if context.get('relevant_protocols'):
            ...
            for protocol in context['relevant_protocols'][:3]:
                formatted.append(f"- {protocol.get('metadata', {}).get('protocol', 'Unknown')}: {protocol.get('content', '')[:200]}...")
    ...
    def _generate_followup_suggestions(self, query_type: str, original_query: str) -> List[str]:
        ...
        suggestions = base_suggestions.get(query_type, [
