...
class ProtocolHandlerGenerator:
    ...
    async def generate_handler(
        ...
        try:
            self.logger.info(
                f"Generating {component_type.value} in {language.value} " f"for protocol {protocol_spec.protocol_name}"
            )

            # Generate code using LLM
            source_code, dependencies, security_notes, rationale = await self._generate_code_llm(
                protocol_spec, language, component_type
            )

            # Generate tests
            test_code = await self._generate_tests(protocol_spec, language, component_type, source_code)

            # Generate documentation
            documentation = self._generate_documentation(protocol_spec, component_type, source_code)

            # Determine file name
            file_name = self._determine_file_name(protocol_spec.protocol_name, component_type, language)
            ...
            self.logger.info(f"Code generation completed: {artifact_id}, " f"lines={len(source_code.splitlines())}")
    ...
    async def _generate_code_llm(
        ...
        """Generate code using LLM."""
        prompt = self._build_code_generation_prompt(protocol_spec, language, component_type)
        ...
        # Parse LLM response
        source_code, dependencies, security_notes = self._parse_code_response(response.content, language)
...
"""
    ...
    def _parse_code_response(self, llm_response: str, language: ProgrammingLanguage) -> Tuple[str, List[str], List[str]]:
    ...
    def _extract_code_blocks(self, text: str, language: ProgrammingLanguage) -> List[str]:
    ...
    def _format_messages_doc(self, messages: List[ProtocolMessage]) -> str:
        ...
        for msg in messages:
            ...
            for field in msg.fields:
                ...
                doc_lines.append(f"- `{field.name}` ({field.field_type}, {required_str}): {field.description}")
            doc_lines.append("")
