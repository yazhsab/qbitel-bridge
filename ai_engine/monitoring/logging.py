...
class StructuredLogger:
    ...
    def trace_span(
        ...
        try:
            ...
            yield context
            self.debug(f"Completed operation: {operation_name}", 
                      trace_id=trace_id, span_id=span_id, operation=operation_name,
    ...
    def log_protocol_discovery(
        ...
        **kwargs
        ...
        if discovered_protocol:
            self.info(f"Protocol discovered: {discovered_protocol} (confidence: {confidence:.2f})", **log_data)
        else:
    ...
    def log_anomaly_detection(
        ...
        **kwargs
        ...
        if is_anomalous:
            self.warning(f"Anomaly detected: score={anomaly_score:.3f}, threshold={threshold:.3f}", **log_data)
        else:
            self.info(f"No anomaly detected: score={anomaly_score:.3f}, threshold={threshold:.3f}", **log_data)
...
def setup_logging(config: Config) -> None:
    ...
    components = [
        "cronos_ai.engine", "cronos_ai.discovery", "cronos_ai.detection",
        "cronos_ai.anomaly", "cronos_ai.training", "cronos_ai.models",
        "cronos_ai.api", "cronos_ai.monitoring"
    ]
