...
from ..models.database import (
    User, APIKey, UserSession, AuditLog, OAuthProvider, SAMLProvider,
    PasswordResetToken, UserRole, MFAMethod, APIKeyStatus, AuditAction
)
...
class EnterpriseAuthenticationService:
    ...
    async def change_password(
        ...
        await self._log_audit(
            db, user_id, AuditAction.PASSWORD_CHANGE, success=True
        )
    ...
    async def enable_mfa(
        ...
        if method == MFAMethod.TOTP:
            ...
            await self._log_audit(
                db, user_id, AuditAction.MFA_ENABLED,
                success=True, details={"method": method.value}
            )
    ...
    async def create_api_key(
        ...
        await self._log_audit(
            db, user_id, AuditAction.API_KEY_CREATED,
            success=True, details={"key_name": name, "key_id": str(api_key_record.id)}
        )
    ...
    async def revoke_api_key(
        ...
        await self._log_audit(
            db, revoked_by, AuditAction.API_KEY_REVOKED,
            success=True, details={"key_id": key_id, "reason": reason}
        )
    ...
    async def authenticate_user(
        ...
        if user.account_locked_until and user.account_locked_until > datetime.utcnow():
            await self._log_audit(
                db, str(user.id), AuditAction.LOGIN_FAILED,
        ...
        if self._requires_mfa(user):
            if not user.mfa_enabled:
                ...
                if not await self.verify_mfa_token(db, str(user.id), mfa_token):
                    await self._log_audit(
                        db, str(user.id), AuditAction.LOGIN_FAILED,
                        success=False, error_message="Invalid MFA token",
                        ip_address=ip_address
                    )
                    self.audit_logger.log_login_failed(
                        username=username,
        ...
        await self._log_audit(
            db, str(user.id), AuditAction.LOGIN_SUCCESS,
