...
from cloud_native.event_streaming.kafka.threat_detector import (
    KafkaAnomaly,
    KafkaThreatDetector
)
...
class TestKafkaThreatDetector:
    ...
    def test_analyze_stream_high_message_rate(self, detector):
        ...
        anomalies = detector.analyze_stream(
            topic="test-topic",
            message_rate=15000,  # Above threshold (10000)
            error_rate=0.01,
            consumer_lag=100
        )
        ...
        high_rate_anomalies = [
            ...
            if a.anomaly_type == "high_message_rate"
        ]
    ...
    def test_analyze_stream_high_error_rate(self, detector):
        ...
        anomalies = detector.analyze_stream(
            topic="test-topic",
            message_rate=1000,
            error_rate=0.15,  # 15% error rate (above 10% threshold)
            consumer_lag=50
        )
    ...
    def test_analyze_stream_consumer_lag(self, detector):
        ...
        anomalies = detector.analyze_stream(
            ...
            message_rate=1000,
            error_rate=0.01,
            consumer_lag=150000  # Above threshold (100000)
        )
        ...
        lag_anomalies = [
            a for a in anomalies
            if a.anomaly_type == "consumer_lag"
        ]
    ...
    def test_analyze_stream_multiple_anomalies(self, detector):
        ...
        anomalies = detector.analyze_stream(
            topic="test-topic",
            message_rate=20000,  # High
            error_rate=0.20,      # High
            consumer_lag=200000   # High
        )
    ...
    def test_analyze_stream_no_anomalies(self, detector):
        ...
        anomalies = detector.analyze_stream(
            topic="test-topic",
            message_rate=1000,    # Normal
            error_rate=0.01,      # Normal
            consumer_lag=100      # Normal
        )
    ...
    def test_threshold_customization(self):
        ...
        detector = KafkaThreatDetector(
            message_rate_threshold=5000,
            error_rate_threshold=0.05,
            consumer_lag_threshold=50000
        )
