...
from cloud_native.service_mesh.envoy.xds_server import (
    XDSResourceType,
    XDSResource,
    EnvoyNode,
    EnvoyXDSServer
...
class TestEnvoyXDSServer:
    ...
    async def test_create_route(self, xds_server):
        ...
        route = await xds_server.create_route(
            name="main_route",
    ...
    def test_subscribe(self, xds_server):
        ...
        xds_server.subscribe(
            node_id="envoy-1",
            resource_type=XDSResourceType.LISTENER,
            resource_names=["http_listener", "https_listener"]
        )
    ...
    async def test_delete_resource(self, xds_server):
        ...
        snapshot = xds_server.get_snapshot()
        assert any(
            r["name"] == "temp_listener"
            for r in snapshot.get(XDSResourceType.LISTENER.value, [])
        )
    ...
    async def test_quantum_filter_chain(self, xds_server):
        ...
        quantum_filter = next(
            (f for f in filters if "quantum" in f.get("name", "").lower()),
            None
        )
