...
from ai_engine.core.error_handling import (
    ...
    RecoveryStrategy, ErrorContext, CircuitBreaker, CircuitBreakerConfig
)
...
class TestConfigValidator:
    ...
    def test_security_validation(self):
        ...
        tls_errors = [
            issue for issue in validator.issues
            if issue.severity == ValidationSeverity.ERROR and "tls" in issue.field.lower()
        ]
    ...
    def test_hardcoded_secrets_detection(self):
        ...
        config = {
            "database": {
                "password": "hardcoded-password"  # Should be detected
            },
            "security": {
                "jwt_secret": "my-secret-key"  # Should be detected
            }
        }
        ...
        secret_errors = [
            issue for issue in validator.issues
            if "hardcoded" in issue.message.lower()
        ]
...
class TestIntegration:
    ...
    async def test_full_error_handling_flow(self):
        ...
        try:
            ...
            recovery_successful, result = await handler.handle_error(
                e, context, None
            )
