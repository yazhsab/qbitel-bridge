...
class TestCertificateValidation:
    ...
    def test_valid_certificate_chain(self, root_ca_cert_and_key,
                                     intermediate_ca_cert_and_key,
                                     end_entity_cert_and_key):
        ...
        root_public_key.verify(
            ...
            ec.ECDSA(hashes.SHA256()) if isinstance(root_public_key, ec.EllipticCurvePublicKey)
            else hashes.SHA256()
        )
        ...
        intermediate_public_key.verify(
            ...
            ec.ECDSA(hashes.SHA256()) if isinstance(intermediate_public_key, ec.EllipticCurvePublicKey)
            else hashes.SHA256()
        )
    ...
    def test_path_length_constraint(self, root_ca_cert_and_key,
                                    intermediate_ca_cert_and_key):
        ...
        intermediate_bc = intermediate_cert.extensions.get_extension_for_oid(
            ExtensionOID.BASIC_CONSTRAINTS
        )
        assert intermediate_bc.value.path_length == 0, \
            "Intermediate CA should have path_length=0"
    ...
    def test_weak_signature_algorithm_detection(self, root_ca_cert_and_key):
        ...
        assert isinstance(weak_cert.signature_hash_algorithm, hashes.SHA1), \
    ...
    def test_certificate_revocation_list_structure(self, root_ca_cert_and_key):
        ...
        # Create a CRL
        revoked_cert_1 = x509.RevokedCertificateBuilder() \
            .serial_number(12345) \
            .revocation_date(datetime.utcnow()) \
            .build(default_backend())

        revoked_cert_2 = x509.RevokedCertificateBuilder() \
            .serial_number(67890) \
            .revocation_date(datetime.utcnow()) \
            .build(default_backend())
    ...
    def test_certificate_chain_order(self, root_ca_cert_and_key,
                                     intermediate_ca_cert_and_key,
                                     end_entity_cert_and_key):
        ...
        for i in range(len(chain) - 1):
            ...
            assert current_cert.issuer == issuer_cert.subject, \
                f"Certificate {i} issuer mismatch"
