    ...
    async def enhanced_orchestrator(self, mock_config, mock_copilot):
        ...
        with (
            ...
            orchestrator._execute_discovery = AsyncMock(
                return_value=EnhancedDiscoveryResult(
                    protocol_type="HTTP",
                    confidence=0.8,
                    processing_time=1.5,
                    phases_completed=[],
...
class TestAPIIntegration:
    ...
    async def test_copilot_endpoint_processing(self):
        ...
        # Mock the copilot
        mock_copilot = Mock()
        mock_copilot.process_query = AsyncMock(
            return_value=CopilotResponse(
...
async def integration_config():
    ...
    return {
        ...
        "redis": {"host": "localhost", "port": 6379, "db": 1},  # Use test database
        "security": {"jwt_secret": "test-secret-key", "enable_rate_limiting": False},
    }
