...
class TestRESTAPI:
    ...
    def test_protocol_discovery_success(self, client, auth_headers, sample_data):
        ...
        mock_engine = SimpleNamespace(
            discover_protocol=AsyncMock(return_value={
                "protocol_type": "http",
                "confidence": 0.92,
                "structure": {"fields": []},
    ...
    def test_protocol_discovery_invalid_data(self, client, auth_headers):
        ...
        payload = {
            "packet_data": "invalid_base64_data!@#",
            "metadata": None
        }
        response = client.post("/api/v1/discover", json=payload, headers=auth_headers)
    ...
    def test_field_detection_success(self, client, auth_headers, sample_data):
        ...
        mock_engine = SimpleNamespace(
            detect_fields=AsyncMock(return_value=[
                {"id": "field_1", "name": "method", "start": 0, "end": 3, "type": "method", "confidence": 0.95},
                {"id": "field_2", "name": "path", "start": 4, "end": 17, "type": "path", "confidence": 0.88}
            ])
        )
    ...
    def test_app_startup_initializes_services(self, config):
        with ExitStack() as stack:
            mock_engine_cls = stack.enter_context(patch('ai_engine.api.rest.CronosAIEngine'))
            ...
            mock_create_copilot = stack.enter_context(patch('ai_engine.api.rest.create_protocol_copilot', new_callable=AsyncMock))
            mock_metrics_cls = stack.enter_context(patch('ai_engine.api.rest.MetricsCollector'))
            mock_init_translation = stack.enter_context(patch('ai_engine.api.rest.initialize_translation_studio', new_callable=AsyncMock))
            mock_shutdown_translation = stack.enter_context(patch('ai_engine.api.rest.shutdown_translation_studio', new_callable=AsyncMock))
            mock_init_alert = stack.enter_context(patch('ai_engine.api.rest.initialize_alert_manager', new_callable=AsyncMock))
            mock_shutdown_alert = stack.enter_context(patch('ai_engine.api.rest.shutdown_alert_manager', new_callable=AsyncMock))
            mock_get_policy = stack.enter_context(patch('ai_engine.api.rest.get_policy_engine'))
            mock_init_security = stack.enter_context(patch('ai_engine.api.rest.initialize_security_orchestrator', new_callable=AsyncMock))
            mock_shutdown_security = stack.enter_context(patch('ai_engine.api.rest.shutdown_security_orchestrator', new_callable=AsyncMock))
...
class TestGRPCAPI:
    ...
    async def test_service_status_grpc(self, grpc_service):
        ...
        engine = SimpleNamespace(
            _initialized=True,
            get_model_info=lambda: mock_engine_status
        )
        grpc_service.ai_engine = engine
...
class TestAPIAuthentication:
    ...
    def test_invalid_api_key(self, client, sample_data):
        ...
        payload = {
            "packet_data": sample_data["base64_http"],
            "metadata": None
        }
