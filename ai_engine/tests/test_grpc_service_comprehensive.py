...
class TestAIEngineGRPCService:
    ...
    async def test_discover_protocol(self, service):
        ...
        service.ai_engine.discover_protocol = AsyncMock(return_value={
            "protocol_type": "http",
            "confidence": 0.95,
    ...
    async def test_detect_anomalies(self, service):
        ...
        service.ai_engine = AsyncMock()
        service.ai_engine.detect_anomaly = AsyncMock(return_value={
            "is_anomaly": True,
            "anomaly_score": 0.85,
            "confidence": 0.9,
    ...
    async def test_batch_process_with_failures(self, service):
        ...
        service.ai_engine = AsyncMock()
        service.ai_engine.discover_protocol = AsyncMock(side_effect=[
            {"protocol_type": "http"},
            Exception("Failed"),
            {"protocol_type": "tcp"}
        ])
