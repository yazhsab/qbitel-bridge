...
async def translation_studio(config, mock_llm_service):
    ...
    studio = ProtocolTranslationStudio(config=config, llm_service=mock_llm_service)
    return studio
...
class TestProtocolTranslationStudio:
    ...
    async def test_processing_step_trims_and_lowercases(self, translation_studio):
        ...
        lowered = await translation_studio._apply_processing_step(
            trimmed, json.dumps({"operation": "lowercase", "fields": ["method"]})
        )
    ...
    async def test_apply_translation_rules(
        ...
        source_data = {"method": "GET", "path": "/api/test"}
    ...
    async def test_performance_metrics(self, translation_studio):
        ...
        translation_studio._record_translation_metrics("http", "mqtt", 1.5, True)
    ...
    async def test_transformation_application(self, translation_studio):
        ...
        result = await translation_studio._apply_transformation(10, "value * 2")
        assert result == 20

        # Test string transformation
        result = await translation_studio._apply_transformation("test", "value.upper()")
        assert result == "TEST"
    ...
    async def test_condition_checking(self, translation_studio):
        ...
        result = await translation_studio._check_conditions(data, ["field1 < 5"])
        assert result is False
    ...
    async def test_bottleneck_analysis(self, translation_studio, http_spec, mqtt_spec):
        ...
        rules = TranslationRules(
            ...
            rules=[
                ...
                for i in range(150)
            ],  # Many rules to trigger bottleneck
        )
...
class TestIntegration:
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_translation(self, translation_studio, mock_llm_service):
