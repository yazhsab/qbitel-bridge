...
class TestModelTrainer:
                ...
                with patch('mlflow.log_metrics'):
                    with patch('mlflow.pytorch.log_model'):
                        with patch.object(trainer, '_prepare_model_for_training', return_value=model):
                            with patch.object(trainer, '_training_loop', return_value={"train_loss": [0.5], "best_val_loss": 0.4}):
        ...
        with patch('mlflow.start_run'):
            with patch('mlflow.log_params'):
                with patch('mlflow.log_metrics'):
                    with patch('mlflow.pytorch.log_model'):
                        with patch.object(trainer, '_prepare_model_for_training', return_value=model):
                            with patch.object(trainer, '_training_loop', return_value={"train_loss": [0.5], "val_loss": [0.4], "best_val_loss": 0.4}):
                                with patch.object(trainer, '_save_best_model', return_value="/tmp/model.pt"):
                                    job = await trainer.train_model(
                                        model=model,
                                        train_dataloader=train_loader,
    ...
    async def test_train_model_failure(self, trainer):
        ...
        with patch('mlflow.start_run'):
            with patch.object(trainer, '_prepare_model_for_training', side_effect=Exception("Training error")):
                with pytest.raises(TrainingException):
