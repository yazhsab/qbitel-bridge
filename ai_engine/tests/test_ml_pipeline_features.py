...
class TestFieldDetectorValidationMetrics:
    ...
    async def test_training_history_completeness(self, trained_detector):
        ...
        expected_keys = [
            'train_loss', 'val_loss', 'val_f1', 'val_precision',
            'val_recall', 'val_accuracy', 'learning_rate', 'epoch_time', 'summary'
        ]
...
class TestFieldDetectorModelPersistence:
    ...
    async def test_checkpoint_contains_full_state(self, trained_detector):
        ...
        required_keys = [
            'epoch', 'model_state_dict', 'version', 'timestamp',
            'metrics', 'config', 'tag_mappings'
        ]
...
class TestTranslationStudioBatchTraining:
    ...
    async def translation_studio(self):
        ...
        llm_service.process_request = AsyncMock(return_value=Mock(
            content='{"rule_modifications": []}'
        ))
    ...
    def sample_protocols(self):
        ...
        source_spec = ProtocolSpecification(
            ...
            fields=[
                ProtocolField(name="field1", field_type=TransFieldType.INTEGER, length=4, required=True),
                ProtocolField(name="field2", field_type=TransFieldType.STRING, length=16, required=True)
            ]
        )
    ...
    async def test_validation_metrics_comprehensive(self, translation_studio, sample_protocols):
        ...
        expected_keys = [
            'pass_rate', 'fail_rate', 'error_rate', 'avg_latency_ms',
            'p50_latency_ms', 'p95_latency_ms', 'total_examples',
...
class TestRollbackFunctionality:
    ...
    async def test_translation_studio_rollback(self, tmp_path):
        ...
        source_spec = ProtocolSpecification(
            protocol_id="test", protocol_type=ProtocolType.CUSTOM,
            version="1.0", name="Test", description="Test", fields=[]
        )
        target_spec = ProtocolSpecification(
            protocol_id="test2", protocol_type=ProtocolType.CUSTOM,
            version="1.0", name="Test2", description="Test", fields=[]
        )
