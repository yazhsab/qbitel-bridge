...
from ai_engine.cloud_native.cloud_integrations.aws.security_hub import AWSSecurityHubIntegration, FindingImportError
...
class TestAWSSecurityHubIntegration:
    ...
    def test_publish_finding_success(self, security_hub, mock_boto_client):
        ...
        # Setup mock response
        mock_boto_client.batch_import_findings.return_value = {"FailedCount": 0, "SuccessCount": 1}

        # Publish finding
        result = security_hub.publish_finding(
            title="Test Finding", description="This is a test finding", severity="HIGH", resource_id="test-resource-123"
        )
    ...
    def test_publish_finding_invalid_severity(self, security_hub):
        ...
        with pytest.raises(ValueError, match="Invalid severity"):
            security_hub.publish_finding(title="Test Finding", description="Test", severity="INVALID", resource_id="test-123")
    ...
    def test_publish_finding_with_failed_count(self, security_hub, mock_boto_client):
        ...
        with pytest.raises(FindingImportError, match="Failed to import finding"):
            security_hub.publish_finding(title="Test Finding", description="Test", severity="HIGH", resource_id="test-123")
    ...
    def test_publish_finding_retry_on_client_error(self, security_hub):
        ...
        with patch("ai_engine.cloud_native.cloud_integrations.aws.security_hub.boto3") as mock_boto:
            ...
            mock_client.batch_import_findings.side_effect = [
                ClientError({"Error": {"Code": "ThrottlingException", "Message": "Rate exceeded"}}, "batch_import_findings"),
                ClientError({"Error": {"Code": "ThrottlingException", "Message": "Rate exceeded"}}, "batch_import_findings"),
            ...
            with patch("time.sleep"):  # Speed up test by mocking sleep
                result = security_hub.publish_finding(
                    title="Test Finding", description="Test", severity="HIGH", resource_id="test-123"
                )
    ...
    def test_publish_finding_max_retries_exceeded(self, security_hub):
        ...
        with patch("ai_engine.cloud_native.cloud_integrations.aws.security_hub.boto3") as mock_boto:
            ...
            mock_client.batch_import_findings.side_effect = ClientError(
                {"Error": {"Code": "ServiceError", "Message": "Service unavailable"}}, "batch_import_findings"
            )
    ...
    def test_batch_publish_findings(self, security_hub, mock_boto_client):
        ...
        # Setup mock response
        mock_boto_client.batch_import_findings.return_value = {"FailedCount": 0, "SuccessCount": 5}
    ...
    def test_batch_publish_findings_multiple_batches(self, security_hub, mock_boto_client):
        ...
        # Setup mock response
        mock_boto_client.batch_import_findings.return_value = {"FailedCount": 0, "SuccessCount": 100}
    ...
    @pytest.mark.parametrize(
        "severity,expected_compliance",
        [("CRITICAL", "FAILED"), ("HIGH", "FAILED"), ("MEDIUM", "WARNING"), ("LOW", "WARNING"), ("INFORMATIONAL", "WARNING")],
    )
    def test_compliance_status_mapping(self, security_hub, mock_boto_client, severity, expected_compliance):
        ...
        security_hub.publish_finding(title="Test", description="Test", severity=severity, resource_id="test-123")
