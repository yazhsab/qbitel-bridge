...
class TestSecretValidation:
    ...
    def test_no_hardcoded_passwords_in_config(self):
        ...
        assert db_config.password == '' or db_config.password is None or \
               os.getenv('CRONOS_AI_DB_PASSWORD') or os.getenv('DATABASE_PASSWORD'), \
    ...
    def test_no_hardcoded_secrets_in_security_config(self):
        ...
        if config.jwt_secret:
            assert os.getenv('CRONOS_AI_JWT_SECRET') or os.getenv('JWT_SECRET'), \
                   "SecurityConfig has hardcoded JWT secret"
        
        # Encryption key should be empty or from environment
        if config.encryption_key:
            assert os.getenv('CRONOS_AI_ENCRYPTION_KEY') or os.getenv('ENCRYPTION_KEY'), \
