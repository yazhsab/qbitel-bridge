...
class TestProductionModeDetector:
    ...
    def test_validate_production_environment_missing_vars(self):
        ...
        with patch.dict(os.environ, {"CRONOS_AI_ENVIRONMENT": "production"}):
            ...
            assert "CRONOS_AI_DB_PASSWORD" in error_text
            assert "CRONOS_AI_REDIS_PASSWORD" in error_text
            assert "CRONOS_AI_JWT_SECRET" in error_text
            assert "CRONOS_AI_ENCRYPTION_KEY" in error_text
