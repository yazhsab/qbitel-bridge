...
class TestSecretsManagerEnvironment:
    ...
    def test_set_secret_not_supported_for_environment(self, manager):
        ...
        with pytest.raises(SecretValidationError, match="Setting secrets not supported"):
...
class TestSecretsManagerVault:
    ...
    def test_init_vault_backend_success(self, mock_vault_client):
        ...
        with patch.dict("sys.modules", {"hvac": MagicMock()}):
            ...
            with patch.dict(os.environ, {"VAULT_ADDR": "http://localhost:8200", "VAULT_TOKEN": "test_token"}):
    ...
    def test_init_vault_missing_addr(self):
        ...
        with patch.dict("sys.modules", {"hvac": MagicMock()}):
            with patch.dict(os.environ, {}, clear=True):
                with pytest.raises(SecretValidationError, match="VAULT_ADDR not configured"):
    ...
    def test_init_vault_not_authenticated(self):
        ...
        with patch.dict("sys.modules", {"hvac": MagicMock()}):
            ...
            with patch.dict(os.environ, {"VAULT_ADDR": "http://localhost:8200"}):
                with pytest.raises(SecretValidationError, match="Vault authentication failed"):
                    SecretsManager({"backend": "vault"})

    def test_init_vault_import_error(self):
        """Test Vault initialization with missing hvac package."""
        with patch.dict("sys.modules", {"hvac": None}):
            with pytest.raises(SecretValidationError, match="hvac package not installed"):
                SecretsManager({"backend": "vault"})
    ...
    def test_get_secret_from_vault(self, mock_vault_client):
        ...
        with patch("ai_engine.security.secrets_manager.hvac.Client", return_value=mock_vault_client):
            with patch.dict(os.environ, {"VAULT_ADDR": "http://localhost:8200", "VAULT_TOKEN": "test_token"}):
                manager = SecretsManager({"backend": "vault"})
                value = manager.get_secret("test_key")
...
class TestSecretsManagerAWS:
    ...
    def test_init_aws_backend_success(self, mock_aws_client):
        ...
        with patch("ai_engine.security.secrets_manager.boto3.client", return_value=mock_aws_client):
            manager = SecretsManager({"backend": "aws_secrets_manager", "aws_region": "us-east-1"})
    ...
    def test_init_aws_import_error(self):
        ...
        with patch("ai_engine.security.secrets_manager.boto3", side_effect=ImportError):
            with pytest.raises(SecretValidationError, match="boto3 package not installed"):
    ...
    def test_set_secret_in_aws_create(self, mock_aws_client):
        ...
        mock_aws_client.update_secret.side_effect = mock_aws_client.exceptions.ResourceNotFoundException()
        with patch("ai_engine.security.secrets_manager.boto3.client", return_value=mock_aws_client):
            manager = SecretsManager({"backend": "aws_secrets_manager"})
            manager.set_secret("test_key", "test_value")
...
class TestSecretsManagerAzure:
    ...
    def test_init_azure_backend_success(self, mock_azure_client):
        ...
        with patch("ai_engine.security.secrets_manager.SecretClient", return_value=mock_azure_client):
            with patch("ai_engine.security.secrets_manager.DefaultAzureCredential"):
                with patch.dict(os.environ, {"AZURE_VAULT_URL": "https://test.vault.azure.net"}):
    ...
    def test_init_azure_missing_url(self):
        ...
        with patch("ai_engine.security.secrets_manager.SecretClient"):
            with patch("ai_engine.security.secrets_manager.DefaultAzureCredential"):
                with patch.dict(os.environ, {}, clear=True):
                    with pytest.raises(SecretValidationError, match="AZURE_VAULT_URL not configured"):
                        SecretsManager({"backend": "azure_key_vault"})

    def test_init_azure_import_error(self):
        """Test Azure initialization with missing SDK."""
        with patch("ai_engine.security.secrets_manager.SecretClient", side_effect=ImportError):
...
class TestSecretsManagerFile:
    ...
    def test_get_encryption_key_existing(self, temp_dir):
        ...
        manager = SecretsManager({"backend": "file", "secrets_file": str(Path(temp_dir) / ".secrets.enc")})
        # Change to temp_dir to use the key file
        with patch("ai_engine.security.secrets_manager.Path", return_value=key_file.parent):
            key = manager._get_encryption_key()
            assert key == test_key

    def test_get_encryption_key_generate_new(self, temp_dir):
        """Test generating new encryption key."""
        with patch("ai_engine.security.secrets_manager.Path.cwd", return_value=Path(temp_dir)):
...
class TestGlobalFunctions:
    ...
    def test_get_secret_convenience_function(self):
        ...
        with patch("ai_engine.security.secrets_manager.get_secrets_manager") as mock_get:
            mock_manager = Mock()
...
class TestSecretsManagerErrorHandling:
    ...
    def test_get_secret_exception_returns_default(self):
        ...
        manager = SecretsManager({"backend": "environment"})
        with patch.object(manager, "_get_from_env", side_effect=Exception("Test error")):
            value = manager.get_secret("test_key", default="default")
            assert value == "default"
    ...
    def test_set_secret_exception_raises(self):
        ...
        mock_client = Mock()
        mock_client.is_authenticated.return_value = True
        mock_client.secrets.kv.v2.create_or_update_secret.side_effect = Exception("Test error")
        
        with patch("ai_engine.security.secrets_manager.hvac.Client", return_value=mock_client):
            with patch.dict(os.environ, {"VAULT_ADDR": "http://localhost:8200", "VAULT_TOKEN": "test"}):
                manager = SecretsManager({"backend": "vault"})
    ...
    def test_delete_secret_exception_raises(self):
        ...
        mock_client = Mock()
        mock_client.is_authenticated.return_value = True
        mock_client.secrets.kv.v2.delete_metadata_and_all_versions.side_effect = Exception("Test error")
