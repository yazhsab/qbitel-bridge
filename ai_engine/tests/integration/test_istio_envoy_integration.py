...
from cloud_native.service_mesh.istio.qkd_certificate_manager import QuantumCertificateManager, CertificateAlgorithm
...
class TestIstioEnvoyIntegration:
    ...
    async def test_mtls_with_quantum_certificates(self):
        ...
        cert_manager = QuantumCertificateManager(
            key_algorithm=CertificateAlgorithm.KYBER_1024, signature_algorithm=CertificateAlgorithm.DILITHIUM_5
        )
        ...
        dest_rule = mtls_config.create_destination_rule(
            name="test-service", host="test-service.default.svc.cluster.local", namespace="default", enable_quantum_tls=True
        )

        # Configure Envoy TLS
        encryption_mgr = TrafficEncryptionManager(enable_quantum_safe=True, min_tls_version=TLSVersion.TLS_1_3)
    ...
    async def test_sidecar_injection_with_xds(self):
        ...
        injector = SidecarInjector(namespace="istio-system", enable_quantum_crypto=True)
        ...
        listener = await xds_server.create_listener(
            name="virtualInbound", address="0.0.0.0", port=15006, enable_quantum_filter=True
        )

        cluster = await xds_server.create_cluster(
            name="outbound|80||nginx.default.svc.cluster.local", endpoints=["127.0.0.1:80"], lb_policy="ROUND_ROBIN"
        )
