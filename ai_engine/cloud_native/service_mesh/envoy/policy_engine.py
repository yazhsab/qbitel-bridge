...
class TrafficPolicyEngine:
    ...
    def create_rate_limit_policy(
        self, service_name: str, requests_per_second: int, burst: Optional[int] = None, per_connection: bool = False
    ) -> TrafficPolicy:
        ...
        config = {
            ...
            "descriptors": [
                {
                    "entries": [{"key": "header_match", "value": service_name}],
                    "rate_limit": {"unit": "SECOND", "requests_per_unit": requests_per_second},
                }
            ],
            "token_bucket": {
        ...
        policy = TrafficPolicy(
            name=f"{service_name}_rate_limit", policy_type=PolicyType.RATE_LIMIT, target_service=service_name, config=config
        )
    ...
    def create_retry_policy(
        ...
        """Create retry policy"""
        config = {
            "retry_on": ",".join(retry_on or ["5xx", "gateway-error", "reset", "connect-failure", "refused-stream"]),
            ...
            "retry_host_predicate": (
                ...
                if not retry_host_predicate
                else retry_host_predicate
            ),
        }

        policy = TrafficPolicy(
            name=f"{service_name}_retry", policy_type=PolicyType.RETRY, target_service=service_name, config=config
        )
    ...
    def create_timeout_policy(
        self, service_name: str, request_timeout: str = "30s", idle_timeout: str = "300s", stream_idle_timeout: str = "300s"
    ) -> TrafficPolicy:
        ...
        config = {
            ...
            "max_stream_duration": {"max_stream_duration": "300s", "grpc_timeout_header_max": "300s"},
        }

        policy = TrafficPolicy(
            name=f"{service_name}_timeout", policy_type=PolicyType.TIMEOUT, target_service=service_name, config=config
        )
    ...
    def create_load_balancer_policy(
        ...
        policy = TrafficPolicy(
            name=f"{service_name}_lb", policy_type=PolicyType.LOAD_BALANCER, target_service=service_name, config=config
        )
    ...
    def get_policies_for_service(self, service_name: str) -> List[TrafficPolicy]:
        ...
        return [policy for policy in self._policies.values() if policy.target_service == service_name]
