...
AGENT_ACTIONS = Counter(
    "qbitel_agent_actions_total",
    "Total agent actions",
    ["agent_role", "action_type", "status"]
)
AGENT_TOOL_CALLS = Counter(
    "qbitel_agent_tool_calls_total",
    "Total tool calls by agents",
    ["agent_role", "tool_name", "status"]
)
AGENT_REFLECTION_COUNT = Counter(
    "qbitel_agent_reflections_total",
    "Total agent reflections",
    ["agent_role", "reflection_type"]
)
AGENT_RESPONSE_TIME = Histogram(
    "qbitel_agent_response_seconds",
    "Agent response time",
    ["agent_role"]
)
AGENT_TOKEN_USAGE = Counter(
    "qbitel_agent_tokens_total",
    "Total tokens used by agents",
    ["agent_role", "token_type"]  # input, output
)
...
Please provide a helpful and accurate answer."""
    ...
    async def _handle_feedback(self, message: AgentMessage) -> AgentMessage:
        ...
        self.memory.add_long_term(
            f"Feedback on task {message.parent_id}: {message.content}",
            importance=0.9
        )
    ...
    async def _handle_generic(self, message: AgentMessage) -> AgentMessage:
        ...
        response = await self._call_llm(
            f"Respond to this message: {message.content}"
        )
