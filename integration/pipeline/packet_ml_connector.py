...
from integration.streaming.kafka_streaming_service import get_streaming_service, StreamEvent, StreamEventType
...
class PacketMLConnector:
    ...
    def __init__(self):
        ...
        self.packet_processor = ThreadPoolExecutor(
            max_workers=8, 
            thread_name_prefix="packet_proc_"
        )
        self.analysis_executor = ThreadPoolExecutor(
            max_workers=4, 
            thread_name_prefix="packet_analysis_"
        )
        ...
        try:
            ...
            self.packet_filters = [
                ...
                },
                {
                    ...
                    'condition': lambda pkt: self._is_suspicious_port(pkt.dest_port) or self._is_suspicious_port(pkt.source_port),
                    'enabled': True,
                ...
                if self.rust_lib:
                    ...
                    if result == 0 and packet_data.data_len > 0:
                        ...
                        try:
                            ...
                            ).result(timeout=0.001)  # 1ms timeout
                        except:
    ...
    async def _batch_processor(self):
        ...
        while self.running:
            try:
                ...
                batch_ready = (
                    len(packet_batch) >= self.batch_size or
    ...
    async def _metrics_collector(self):
        ...
        while self.running:
            try:
                ...
                await self.streaming_service.send_event('cronos-ai-metrics', metrics_event)
    ...
    async def add_packet_filter(self, name: str, condition: Callable, priority: Optional[ProcessingPriority] = None):
