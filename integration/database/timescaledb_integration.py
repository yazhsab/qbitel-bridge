...
class TimescaleDBIntegration:
    ...
    async def _buffer_flush_task(self):
        ...
        while self.running:
            try:
                ...
                should_flush = (
                    len(self.metrics_buffer) >= self.max_buffer_size or
                    len(self.logs_buffer) >= self.max_buffer_size or
                    len(self.security_buffer) >= self.max_buffer_size or
                    len(self.protocol_buffer) >= self.max_buffer_size or
    ...
    async def query_metrics(self, metric_name: str, start_time: datetime, 
                          end_time: datetime, labels: Optional[Dict[str, str]] = None) -> List[Dict[str, Any]]:
        """Query metrics data"""
        try:
            async with self.pool.acquire() as conn:
                query = '''
    ...
    async def query_security_events(self, start_time: datetime, end_time: datetime,
    ...
    async def get_protocol_analysis_stats(self, start_time: datetime, 
