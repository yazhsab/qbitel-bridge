...
class BackupMonitor:
    ...
    async def check_backup_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Backup health check completed: {health_report['status']}")
...
class BackupAutomation:
    ...
    async def run_disaster_recovery_drill(self) -> Dict[str, Any]:
        ...
        try:
            ...
            verify_success = await self.backup_manager.verify_backup(latest_backup.backup_id)
            drill_results["tests"].append({
                "name": "verify_backup",
            ...
            drill_results["tests"].append({
                "name": "health_check",
                "status": "passed" if health_report["status"] in ["healthy", "degraded"] else "failed",
        ...
        self.logger.info(f"Disaster recovery drill completed: {drill_results['status']}")
