    ...
    async def verify_backup(self, backup_id: str) -> bool:
        ...
        try:
            ...
            if current_checksum != metadata.checksum:
                raise ValueError(f"Checksum mismatch: expected {metadata.checksum}, got {current_checksum}")
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                if metadata.encrypted:
                    ...
                    self.encryption.decrypt_file(metadata.destination_path, temp_decrypted)
...
async def main():
    ...
    success = await manager.restore_backup(
        ...
        restore_path="/tmp/restored_data"
    )
