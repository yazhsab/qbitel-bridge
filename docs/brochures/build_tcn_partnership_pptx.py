"""
Build QBITEL Bridge x TCN Partnership Brief — editable PowerPoint deck.

Generates a 16:9 widescreen .pptx with enterprise-grade shape-based diagrams
(no ASCII art). Designed to be opened in PowerPoint / Keynote / Google Slides
and freely edited by the sales team before the VP-Tech meeting.
"""
...
def _apply_shadow(shape, blur=6, distance=2, alpha=40000):
    """Soft drop shadow via raw OOXML."""
    sppr = shape.shadow._element
    # The shadow object exists; we add an outerShdw effect to spPr
    sp_pr = shape.fill._xPr  # spPr element
    # Remove any existing effect list to avoid stacking
    ...
    etree.SubElement(color, qn('a:alpha'), {'val': str(alpha)})
...
def slide_cover(prs):
    ...
    # Decorative corner — gold triangle (right-angle)
    gold_tri = slide.shapes.add_shape(
        MSO_SHAPE.RIGHT_TRIANGLE, Inches(9.5), 0, Inches(3.8), Inches(2.5))
    set_fill(gold_tri, GOLD)
    no_line(gold_tri)
    gold_tri.rotation = 90

    # Teal accent rectangle
    teal_acc = slide.shapes.add_shape(
        MSO_SHAPE.RECTANGLE, Inches(11), 0, Inches(2.333), Inches(0.6))
...
def slide_architecture(prs):
    ...
    for i, (num, lbl, c) in enumerate(seam_labels):
        ...
        arr = slide.shapes.add_shape(
            MSO_SHAPE.PENTAGON, x, conn_y, seam_w, conn_h)
        arr.rotation = 90
        set_fill(arr, c)
        no_line(arr)
        # Label box
        lblbox = slide.shapes.add_textbox(
            x, conn_y + Inches(0.1), seam_w, conn_h - Inches(0.2))
        ...
        r1.font.size = Pt(13)
        r1.font.bold = True
        r1.font.color.rgb = WHITE if c != GOLD else NAVY
        p2 = tf.add_paragraph()
        p2.alignment = PP_ALIGN.CENTER
        for ln in lbl.split('\n'):
            run = p2.add_run() if p2.runs else p2.add_run()
            run.text = ln + '\n'
            run.font.name = FONT
            run.font.size = Pt(9)
            run.font.color.rgb = WHITE if c != GOLD else NAVY
            run.font.bold = True
...
def slide_flow_outbound(prs):
    ...
    # Decision diamond (left, below)
    diamond = slide.shapes.add_shape(
        MSO_SHAPE.DIAMOND, Inches(2.8), Inches(3.3),
        Inches(2.2), Inches(1.4))
    set_fill(diamond, GOLD)
    no_line(diamond)
    set_text(diamond, 'Pattern\ndetected\nin <1 sec?',
    ...
    # YES arrow
    arrow(slide, Inches(3.9), Inches(4.7),
          Inches(3.0), Inches(5.3), color=RED, weight=2.5)
    yes_lbl = slide.shapes.add_textbox(
        Inches(3.0), Inches(4.75), Inches(0.6), Inches(0.3))
    set_text(yes_lbl, 'YES', size=10, bold=True, color=RED,
             align=PP_ALIGN.LEFT, anchor=MSO_ANCHOR.TOP)
    ...
    no_box = rounded_box(slide, Inches(6.3), Inches(5.3),
                         ...
                         font_size=12, bold=True, font_color=WHITE,
                         shadow=True)
    arrow(slide, Inches(5.0), Inches(4.4),
          Inches(7.3), Inches(5.3), color=GREEN, weight=2.5)
    no_lbl = slide.shapes.add_textbox(
        Inches(5.6), Inches(4.5), Inches(0.6), Inches(0.3))
    set_text(no_lbl, 'NO', size=10, bold=True, color=GREEN,
             align=PP_ALIGN.LEFT, anchor=MSO_ANCHOR.TOP)
...
def build_pptx():
    ...
    out_path = 'docs/brochures/QBITEL_TCN_Partnership_Brief.pptx'
    prs.save(out_path)
