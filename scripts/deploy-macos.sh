#!/bin/bash
#
# CRONOS AI - macOS Deployment Script
#
# Comprehensive deployment script for running CRONOS AI on macOS
...
CONFIG_FILE="$PROJECT_ROOT/config/cronos_ai.yaml"
...
setup_environment_file() {
    ...
    if [ -f "$PROJECT_ROOT/.env.example" ]; then
        ...
        cat > "$ENV_FILE" << 'EOF'
# CRONOS AI - Environment Configuration
...
start_native_services() {
    ...
    if ! docker ps | grep -q cronos-postgres; then
        ...
        docker run -d \
            ...
            -p 5432:5432 \
            -v cronos-postgres-data:/var/lib/postgresql/data \
            postgres:15-alpine 2>/dev/null || docker start cronos-postgres
    fi
    ...
    if ! docker ps | grep -q cronos-redis; then
        ...
        docker run -d \
            --name cronos-redis \
            -p 6379:6379 \
            -v cronos-redis-data:/data \
            redis:7-alpine 2>/dev/null || docker start cronos-redis
    fi
...
start_application() {
    log_step "Starting CRONOS AI application..."
    ...
    APP_PID=$!
    echo $APP_PID > "$PROJECT_ROOT/.cronos-ai.pid"

    # Wait for application
    sleep 3
    wait_for_service localhost 8000 "CRONOS AI" 30 || {
        log_error "Application failed to start"
        return 1
    }

    log_success "CRONOS AI started (PID: $APP_PID)"
}
...
stop_services() {
    ...
    # Stop application
    if [ -f "$PROJECT_ROOT/.cronos-ai.pid" ]; then
        PID=$(cat "$PROJECT_ROOT/.cronos-ai.pid")
        if ps -p $PID > /dev/null 2>&1; then
            kill $PID 2>/dev/null || true
            log_success "Application stopped"
        fi
        rm -f "$PROJECT_ROOT/.cronos-ai.pid"
    fi
    ...
    if [ "$MODE" = "docker" ]; then
        ...
        docker stop cronos-postgres cronos-redis 2>/dev/null || true
    fi
...
show_status() {
    ...
    docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" | grep -E "cronos|postgres|redis" || echo "  No containers running"
    ...
    if [ -f "$PROJECT_ROOT/.cronos-ai.pid" ]; then
        PID=$(cat "$PROJECT_ROOT/.cronos-ai.pid")
        if ps -p $PID > /dev/null 2>&1; then
            echo -e "  CRONOS AI: ${GREEN}Running${NC} (PID: $PID)"
        else
            echo -e "  CRONOS AI: ${RED}Not Running${NC}"
        fi
    else
        echo -e "  CRONOS AI: ${RED}Not Running${NC}"
    fi
...
show_logs() {
    ...
    if [ "$MODE" = "docker" ]; then
        ...
        if [ -f "/var/log/cronos-ai/app.log" ]; then
...
clean_up() {
    ...
    # Remove containers
    docker rm -f cronos-postgres cronos-redis 2>/dev/null || true

    # Remove volumes
    docker volume rm cronos-postgres-data cronos-redis-data 2>/dev/null || true
...
do_start() {
    ...
    echo ""
    log_success "CRONOS AI is running!"
    echo ""
    echo "Quick test:"
...
show_help() {
    echo "CRONOS AI - macOS Deployment Script"
    echo ""
