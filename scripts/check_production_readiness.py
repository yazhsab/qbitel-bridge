...
class ProductionReadinessChecker:
    ...
    async def run_all_checks(self) -> ReadinessReport:
        ...
        self.logger.info(
            f"Production readiness checks completed. Score: {report.score:.1f}%"
        )
            ...
            if result.returncode == 0:
                ...
                if nodes_result.returncode == 0:
                    ...
                    if (
                        ...
                        and total_nodes >= self.config["thresholds"]["min_replicas"]
                    ):
                        return CheckResult(
    ...
    async def _check_node_resources(self) -> CheckResult:
        ...
        try:
            result = subprocess.run(
                ["kubectl", "top", "nodes"], capture_output=True, text=True, timeout=10
            )
            ...
            if result.returncode == 0:
                ...
                for line in lines:
                    ...
                    if len(parts) >= 5:
                        ...
                        if (
                            memory_percent
                            > self.config["thresholds"]["max_memory_percent"]
                        ):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                failed_pods = [
                    ...
                    in ["Failed", "Unknown", "CrashLoopBackOff"]
                ]
    ...
    async def _check_storage(self) -> CheckResult:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                total_pvs = len(pvs)
                bound_pvs = sum(
                    1 for pv in pvs if pv.get("status", {}).get("phase") == "Bound"
                )
    ...
    async def _check_automated_testing(self) -> CheckResult:
        ...
        has_tests = any(
            test_dir.exists() and list(test_dir.glob("test_*.py"))
    ...
    async def _check_runbooks(self) -> CheckResult:
        ...
        if runbooks_dir.exists():
            runbook_files = list(runbooks_dir.glob("*.go")) + list(
                runbooks_dir.glob("*.py")
            )
    ...
    def _generate_report(self) -> ReadinessReport:
        ...
        score = (
            (summary["pass"] / scoreable_checks * 100) if scoreable_checks > 0 else 0
        )
    ...
    def print_report(self, report: ReadinessReport):
        ...
        if report.recommendations:
            ...
            if len(report.recommendations) > 15:
                print(
                    f"  ... and {len(report.recommendations) - 15} more recommendations"
                )
