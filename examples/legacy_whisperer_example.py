...
async def example_adapter_generation(spec):
    ...
    print(f"  Dependencies: {', '.join(adapter.dependencies) if adapter.dependencies else 'None'}")
...
async def example_behavior_explanation():
    ...
    for risk in explanation.modernization_risks[:3]:
        print(f"    - {risk.get('category', 'unknown')}: {risk.get('description', '')[:60]}...")
...
async def example_complete_workflow():
    ...
    spec = await whisperer.reverse_engineer_protocol(
        traffic_samples=traffic_samples,
        system_context="Legacy command protocol"
    )
    print(f"  ✓ Protocol analyzed: {spec.protocol_name} (confidence: {spec.confidence_score:.2%})")
