...
from ai_engine.compliance.compliance_service import (
    ComplianceService,
...
from ai_engine.compliance.report_generator import (
...
async def compliance_service(test_config, mock_llm_service):
    ...
    with patch(
        "ai_engine.compliance.compliance_service.get_llm_service",
...
class TestComplianceEndToEnd:
    ...
    async def test_full_compliance_workflow(self, compliance_service):
        ...
        try:
            ...
            print(
                f"Assessment completed with {assessment.overall_compliance_score:.1f}% compliance"
            )

            # Step 2: Generate executive summary report
            exec_report_content = await compliance_service.generate_report(
                "PCI_DSS_4_0", ReportType.EXECUTIVE_SUMMARY, ReportFormat.HTML
            )
            ...
            tech_report_content = await compliance_service.generate_report(
                "PCI_DSS_4_0", ReportType.DETAILED_TECHNICAL, ReportFormat.JSON
            )
    ...
    async def test_multi_framework_assessment(self, compliance_service):
        ...
        try:
            ...
            for framework in frameworks:
                ...
                print(
                    f"{framework}: {assessment.overall_compliance_score:.1f}% compliant"
                )
    ...
    async def test_audit_trail_integration(self, compliance_service):
        ...
        try:
            ...
            await compliance_service.generate_report(
                "PCI_DSS_4_0", ReportType.EXECUTIVE_SUMMARY
            )
            ...
            for report_format, expected_type in formats_to_test:
                try:
                    content = await compliance_service.generate_report(
                        "PCI_DSS_4_0", ReportType.EXECUTIVE_SUMMARY, report_format
                    )
                    ...
                    if report_format == ReportFormat.PDF:
                        print(
                            f"PDF generation failed (expected without dependencies): {e}"
                        )
                    else:
                        raise
    ...
    async def test_concurrent_operations(self, compliance_service):
        ...
        try:
            ...
            print(
                f"Concurrent assessments: {len(successful_results)} succeeded out of {len(frameworks)}"
            )
...
class TestCompliancePerformance:
    ...
    async def test_assessment_performance(self, compliance_service):
        ...
        try:
            ...
            print(
                f"Performance test: {successful_count} assessments in {duration:.2f}s"
            )
    ...
    async def test_report_generation_performance(self, compliance_service):
        ...
        try:
            ...
            report_tasks = [
                compliance_service.generate_report(
                    "PCI_DSS_4_0", ReportType.EXECUTIVE_SUMMARY, ReportFormat.HTML
                ),
                compliance_service.generate_report(
                    "PCI_DSS_4_0", ReportType.DETAILED_TECHNICAL, ReportFormat.JSON
